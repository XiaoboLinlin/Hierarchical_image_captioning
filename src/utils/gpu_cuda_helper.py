...
def get_gpus_avail() -> List[Tuple[int, float]]:
    """Get the GPU ids that have memory usage less than or equal to 80%
    """
    memory_usage = get_gpu_memory()

    memory_usage_percnt = [m / 11178 for m in memory_usage]
    cuda_ids = [(i, m) for i, m in enumerate(memory_usage_percnt) if m <= 0.8]
...
def select_device(device: str = "gpu", gpu_index: int = None):
    if device == "cpu":
        return torch.device(device)
    elif device == "gpu":
        # If specific GPU index is provided, use it directly
        if gpu_index is not None:
            print(f"Using GPU {gpu_index} as specified")
            return torch.device(f"cuda:{gpu_index}")
        # Otherwise, auto-select based on availability
